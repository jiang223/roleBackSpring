...
@ValidatePermission
public class SysParmController extends ParentController {
    @Resource
    ParmDao parmDao;
    ...
    public Map getPage(@RequestParam Map map)
    {
        try {
            ...
            //用PageInfo对结果进行包装
            List<Map> list=parmDao.findParmPage(map);
    ...
    {
        try {
            if(parmDao.insertSelective(map)>0){
    ...
    {
        try {
            if(parmDao.updateByPrimaryKeySelective(map)>0){
